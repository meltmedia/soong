...
public abstract class IntegrationTestRunnerService extends Service {
  ...
  private static final String LOG_TAG = "Soong";
  ...
  public void performIntegrationTests() {
    Log.info(LOG_TAG,"### ### ### PERFORMING INTEGRATION TESTS");
    ...
    try {
      ...
      for(Class clazz : getTestClasses()) {
        try {
          ...
          Log.info(LOG_TAG,"successfully setContext on test class " + clazz.getCanonicalName());
        } catch (NoSuchMethodException e) {
          Log.info(LOG_TAG,clazz.getCanonicalName() + " does not accept setContext invokations");
        }
        junit.run(clazz);
      }
      ...
      Log.error(LOG_TAG,"Caught an error while trying to phone home.", e);
    } finally {
      Log.info(LOG_TAG,"### ### ### FINISHED INTEGRATION TESTS");
    }
  }
